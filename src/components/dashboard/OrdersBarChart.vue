...
const series = computed(() => [{ name: 'Pedidos', data: props.data.map((item) => item.orders) }])
</script>

<template>
  <v-card variant="flat" border class="pa-5 h-100">
    <div class="text-subtitle-1 font-weight-bold mb-1">Pedidos por mês</div>
    <div class="text-caption text-medium-emphasis mb-3">Volume mensal no primeiro semestre</div>
